...
`ASRC-002`: implement a scoring-mesh MVP with Geant4 runtime plumbing and deterministic artifact output.
...
| ASRC-002 | P0 | Runtime | Implement a scoring-mesh MVP with Geant4 runtime plumbing and deterministic artifact output | NEXT | Prefer one compact mesh workflow that produces a stable output artifact suitable for focused regression |
| ASRC-003 | P1 | UI | Add UI surfaces to create, inspect, and revise scoring meshes and basic tally settings | PENDING | Keep the first UI narrow and inspector-friendly rather than building a broad analysis dashboard immediately |
