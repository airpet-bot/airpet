...
def test_optical_surface_properties_are_attached_at_runtime():
    """Running a macro with optical-surface property commands must attach them."""
    executable = os.path.join(os.getcwd(), "geant4", "build", "airpet-sim")
    assert os.path.exists(executable), f"Executable not found: {executable}"
...
/g4pet/detector/readFile geometry.gdml
/g4pet/detector/addOpticalSurface MirrorSurf|glisur|polished|dielectric_dielectric|0.95
/g4pet/detector/addOpticalSurfacePropertyConst MirrorSurf|REFLECTIVITY|0.98
/g4pet/detector/addOpticalSurfaceProperty MirrorSurf|EFFICIENCY|2|0.001|0.5|0.002|0.8
/g4pet/detector/addSkinSurface Skin1|box_lv|MirrorSurf
/run/initialize
/exit
"""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        combined_output = result.stdout + result.stderr
        assert "Attached const property 'REFLECTIVITY' = 0.98 to optical surface 'MirrorSurf'" in combined_output, (
            f"Const property not attached. Output:\n{combined_output}"
        )
        assert "Attached property vector 'EFFICIENCY' with 2 pairs to optical surface 'MirrorSurf'" in combined_output, (
            f"Vector property not attached. Output:\n{combined_output}"
        )
