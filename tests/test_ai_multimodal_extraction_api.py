...
def test_artifact_planning_execute_route_emits_failed_execution_parity_warning_when_preflight_drifts_without_applied_operations(client, tmp_path):
    pm = _build_project_manager(tmp_path)
    ...
    failed_batch_result = {
        'success': False,
        'batch_results': [
            {'success': False, 'error': 'Dimension mutation rejected by harness.'},
            {'success': False, 'error': 'Dimension mutation rejected by harness.'},
            {'success': False, 'error': 'Material mutation rejected by harness.'},
        ],
    }
    ...
    with patch('app.get_project_manager_for_session', return_value=pm):
        ...
        with patch.object(pm, 'run_preflight_checks', side_effect=[baseline_report, candidate_report]) as preflight_mock:
            with patch('app.dispatch_ai_tool', return_value=failed_batch_result) as dispatch_mock:
                execute_response = client.post(
                    f"/api/ai/artifacts/{artifact['artifact_id']}/planning/execute",
                    json={
                        'extraction': extraction['extraction'],
    ...
    assert execution['summary']['applied_operation_count'] == 0
    assert execution['summary']['failed_operation_count'] == 3
    assert [entry['status_code'] for entry in execution['operation_results']] == [
        'operation_failed',
        'operation_failed',
        'invalid_material_ref',
    ...
    assert parity_report['summary']['affected_operation_group_count'] == 2
    assert [group['group_id'] for group in parity_report['operation_groups']] == [
        'dimension_hints',
        'material_updates',
    ]
