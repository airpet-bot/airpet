...
def test_generate_macro_emits_probe_mesh_commands(tmp_path):
    """Verify that generate_macro_file emits /score/create/probe and related
    commands for probe scoring meshes, and omits mesh geometry commands."""
    pm = ProjectManager(ExpressionEvaluator())
    ...
    scoring_payload = {
        ...
        "tally_requests": [
            {
                "tally_id": "tally_probe",
                "name": "probe_energy",
                "mesh_ref": {"mesh_id": "mesh_probe"},
                "quantity": "energy_deposit",
            }
        ],
    }
    ...
    macro_path = Path(
        pm.generate_macro_file(
            "probe-mesh-job", {}, str(tmp_path), str(tmp_path), str(version_dir)
        )
    )
    macro_text = macro_path.read_text(encoding="utf-8")

    # Probe mesh commands
    assert "/score/create/probe probe_mesh 2.5 cm 1" in macro_text
    assert "/score/quantity/energyDeposit probe_energy" in macro_text

    # Box mesh still works alongside probe
    assert "/score/create/boxMesh box_mesh" in macro_text
    assert "/score/mesh/boxSize 5 5 5 mm" in macro_text
...
def test_generate_macro_maps_additional_quantities_to_g4_commands(tmp_path):
    """Verify that the 5 G4CAP-011 tally quantities map to correct /score/quantity/ commands."""
    pm = ProjectManager(ExpressionEvaluator())

    scoring_payload = {
        "scoring_meshes": [
            {
                "mesh_id": "mesh_g4cap011",
                "name": "g4cap011_mesh",
                "geometry": {"size_mm": {"x": 20, "y": 20, "z": 20}},
                "bins": {"x": 10, "y": 10, "z": 10},
            }
        ...
        "tally_requests": [
            {
                "tally_id": "t_cellCharge",
                "name": "cellCharge_tally",
                "mesh_ref": {"mesh_id": "mesh_g4cap011"},
                "quantity": "cell_charge",
            },
            {
                "tally_id": "t_passageCellCurrent",
                "name": "passageCellCurrent_tally",
                "mesh_ref": {"mesh_id": "mesh_g4cap011"},
                "quantity": "passage_cell_current",
            },
            {
                "tally_id": "t_volumeFlux",
                "name": "volumeFlux_tally",
                "mesh_ref": {"mesh_id": "mesh_g4cap011"},
                "quantity": "volume_flux",
            },
            {
                "tally_id": "t_nOfTerminatedTrack",
                "name": "nOfTerminatedTrack_tally",
                "mesh_ref": {"mesh_id": "mesh_g4cap011"},
                "quantity": "n_of_terminated_track",
            },
            {
                "tally_id": "t_minKinEAtGeneration",
                "name": "minKinEAtGeneration_tally",
                "mesh_ref": {"mesh_id": "mesh_g4cap011"},
                "quantity": "min_kin_e_at_generation",
            },
        ],
    }
    ...
    macro_path = Path(
        pm.generate_macro_file(
            "g4cap011-job", {}, str(tmp_path), str(tmp_path), str(version_dir)
        )
    )
    macro_text = macro_path.read_text(encoding="utf-8")

    assert "/score/quantity/cellCharge cellCharge_tally" in macro_text
    assert "/score/quantity/passageCellCurrent passageCellCurrent_tally" in macro_text
    assert "/score/quantity/volumeFlux volumeFlux_tally" in macro_text
    assert "/score/quantity/nOfTerminatedTrack nOfTerminatedTrack_tally" in macro_text
    assert "/score/quantity/minKinEAtGeneration minKinEAtGeneration_tally" in macro_text
...
def test_generate_macro_maps_final_five_quantities_to_g4_commands(tmp_path):
    """Verify that the last 5 missing tally quantities map to correct /score/quantity/ commands."""
    pm = ProjectManager(ExpressionEvaluator())
    ...
    macro_path = Path(
        pm.generate_macro_file(
            "final-qty-job", {}, str(tmp_path), str(tmp_path), str(version_dir)
        )
    )
    macro_text = macro_path.read_text(encoding="utf-8")
